var score_h1 = document.querySelector("#score-h1");
var player_health_div = document.querySelector(".health-bar-player");
var player_health = 5;
var enemies_health_div = document.querySelector(".health-bar-enemies");
var enemies_health = 2;
var level_h1 = document.querySelector("#level");
var level = 1;
player_health_div.style.width = "".concat(player_health * 80, "px");
var question_h1 = document.querySelector("#question");
var time_h1 = document.querySelector(".time");
var answer1 = document.querySelector(".answer-button1");
var answer2 = document.querySelector(".answer-button2");
var enemies_div = document.querySelector(".enemies-div");
var level_1_questions = [
    {
        number: 1,
        question: "5 + 5 = ?",
        answer1: "55",
...
function updateQuestions(level) {
    var questionPool = [];
    if (level === 1) {
        ...
        enemies_health = 1;
        enemies_health_div.style.width = "".concat(enemies_health * 80, "px");
    }
    else if (level === 2) {
    ...
    var random_number = Math.floor(Math.random() * questionPool.length);
    ...
    if (window.location.href ===
        "".concat(window.location.href.replace("fight-arena", "score"))) {
        if (level === 1) {
...
function startTimer(seconds) {
    ...
    time_h1.textContent = "".concat(seconds, "sec.");
    timerInterval = setInterval(function () {
        var timeValue = parseInt(time_h1.textContent.replace("sec.", ""), 10);
        if (timeValue > 0) {
            time_h1.textContent = "".concat(timeValue - 1, " sec.");
        }
        else {
            clearInterval(timerInterval);
            player_health = 0;
            player_health_div.style.width = "".concat(player_health * 80, "px");
            if (player_health === 0) {
                window.location.href = "".concat(window.location.href.replace("fight-arena", "score"));
                player_health = 5;
                level = 1;
...
function attachEventListeners() {
    var random_number = updateQuestions(level);
    var random_helth_percent = Math.floor(Math.random() * 10);
    level_h1.textContent = "Lv. ".concat(level);
    enemies_health_div.style.width = "".concat(enemies_health * 80, "px");
    var questionPool = [];
    if (level === 1) {
        questionPool = level_1_questions;
    ...
    answer1.addEventListener("click", function () {
        if (questionPool[random_number].correct_answer ===
